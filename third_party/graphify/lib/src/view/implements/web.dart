      ...
      window.addEventListener(
        ...
        ((Event event) {
          ...
          try {
            ...
            if (raw.isEmpty || !raw.trimLeft().startsWith('{')) return;
            final decoded = jsonDecode(raw);
            if (decoded is Map<String, dynamic>) {
              if (decoded['type'] != 'node_click') return;
              if (decoded['chartId'] != uid) return;
              widget.onGraphEvent!(decoded);
            }
          } catch (_) {}
        }).toJS,
      );
