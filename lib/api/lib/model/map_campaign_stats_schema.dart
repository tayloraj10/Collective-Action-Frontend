...
class MapCampaignStatsSchema {
  ...
  bool operator ==(Object other) =>
      ...
      other is MapCampaignStatsSchema &&
  ...
  @override
  String toString() =>
      'MapCampaignStatsSchema[campaignId=$campaignId, campaignName=$campaignName, submissionCount=$submissionCount, cleanupCount=$cleanupCount, trashReportCount=$trashReportCount, totalBags=$totalBags, totalPounds=$totalPounds, treePlantingCount=$treePlantingCount, wildflowerPlantingCount=$wildflowerPlantingCount, totalPlantings=$totalPlantings]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "MapCampaignStatsSchema[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "MapCampaignStatsSchema[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<MapCampaignStatsSchema> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<MapCampaignStatsSchema>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = MapCampaignStatsSchema.listFromJson(
          entry.value,
          growable: growable,
        );
      }
