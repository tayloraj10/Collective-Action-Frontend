...
class UsersApi {
  ...
  ///
  /// * [String] userId (required):
  Future<Response> getUserStatsUsersDbUserIdStatsGetWithHttpInfo(String userId,) async {
    // ignore: prefer_const_declarations
    final path = r'/users/db/{user_id}/stats'
      .replaceAll('{user_id}', userId);
  ...
  ///
  /// * [String] userId (required):
  Future<UserStatsSchema?> getUserStatsUsersDbUserIdStatsGet(String userId,) async {
    final response = await getUserStatsUsersDbUserIdStatsGetWithHttpInfo(userId,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'UserStatsSchema',) as UserStatsSchema;
